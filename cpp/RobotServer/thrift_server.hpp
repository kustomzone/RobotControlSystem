 ...
 *
 * For convenience, the THRIFT_SERVER macro can be used to derive from a ThriftServer.
 *
 * For example, for the Thrift service 'MyService', a server can be created with the declaration
 *     class MyServiceServer : public THRIFT_SERVER(MyService)
 */
